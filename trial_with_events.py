...
def get_upcoming_events(district: str, state: str):
    try:
        ...
        prompt = f"""You are a helpful assistant that provides information about upcoming events in India.
Please provide a list of upcoming cultural events, festivals, big parties, and other notable gatherings in {district} district, {state} state, India.
...
5. Expected attendance (if known, otherwise "Not available")
6. Sources/References (if any)
7. Official website or social media (if available)
...
Focus on events within the next 3-6 months. If you don't have specific information for this district, provide general information about major festivals and cultural events typically celebrated in this region. Return at least 3-5 events.
"""
                    ...
                    for idx, event in enumerate(events, 1):
                        with st.expander(f"🎊 {event.get('name', 'Unnamed Event')} - {event.get('date', 'Date TBD')}"):
                            ...
                            with col2:
                                ...
                                st.markdown(f"**👥 Expected Attendance:**")
                                st.info(event.get('expected_attendance', 'Not available'))
